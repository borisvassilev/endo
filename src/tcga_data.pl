...
% correctness of the parsing of the contents of the data files is
% provided.
:- module(tcga_data,
    [load_data/0
    ]).
...
% in each of these files. The functor is used for the name of the
% fact to be added to the database, while the arguments must
% match the type of the converted values within each row.
tcga_code_tables(
    [   code_table(
...
parse_sample_barcode(Sample, S, T, P) :-
    phrase( tcga_barcode:barcode(T, C), Sample ),
    tcga_barcode:extract_barcode(participant, T, C, P),
    atom_codes(S, Sample).

% DCGs for parsing the gene expression file.
% ------------------------------------------
% These are the simple rules for parsing the contents of the gene
% expression profile file. A header line consists of a row name
% _HRN_ and the list of the sample names as strings _Ss_, delimited by
% tabs.
header_line(HRN, Ss) -->
...
exprs([na|Ns]) -->
    ...
    !,
    exprs(Ns).
% Otherwise, we assume and validate that it is a valid number.
exprs([N|Ns]) -->
    "\t",
    string_upto_char(0'\t, S),
    {   number_codes(N, S)
